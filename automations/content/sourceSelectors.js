 ...
 * @returns {Promise<{source: string, sourceRef: string, topicId: string}>}
 */
export async function getTrendSource(supabaseUrl, serviceRoleKey, userId, groqApiKey) {
  ...
  if (existing.length > 0) {
    ...
    return { source: topic.topic, sourceRef: topic.id, topicId: topic.id };
  }
  ...
  if (topics.length === 0) {
    throw new Error("Groq returned no topic suggestions");
  }
  ...
  return { source: first.topic, sourceRef: first.id, topicId: first.id };
}
